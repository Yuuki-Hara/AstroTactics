...
struct BattleView: View {
  ...
  private var currentTarget: Enemy? {
    if let id = targetId,
      let enemy = manager.enemies.first(where: { $0.id == id && $0.currentHP > 0 })
    {
      return enemy
    }
  ...
  var body: some View {
    ZStack {
      ...
      if manager.currentState == .victory || manager.currentState == .defeat {
        RestartView(
          isVictory: manager.currentState == .victory, earnedCredits: manager.earnedCredits,
          onRestart: onRestart)
      }
    }
...
#Preview {
  ...
  let dummyAI = AIJSON(
    type: "random", moves: [MoveJSON(intent: "attack", amount: 1, min: nil, max: nil)])
  let enemy = Enemy(
    category: "alien", name: "エイリアン偵察機", imageName: "alien_scout", maxHP: 30, behaviorAI: dummyAI)
  let relic = Relic(
    id: "energy_core", name: "エナジーコア", description: "戦闘開始時、エナジーを1得る。",
    imageName: "bolt.batteryblock.fill", trigger: RelicTrigger.onBattleStart,
    effectType: RelicEffectType.gainEnergy, amount: 1)
  let deckManager = BattleDeckManager(startingDeck: CardDatabase.startingDeck())
  let battleManager = BattleManager(
    player: player, enemies: [enemy], relics: [relic], deckManager: deckManager)
