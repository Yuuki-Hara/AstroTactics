...
struct ContentView: View {
  ...
  @State private var battleManager: BattleManager?
    ...
    VStack(spacing: 0) {
      ...
      ZStack {
        ...
        case .map:
          MapView(
            runManager: runManager,
            onNodeSelected: { selectedNode in
  ...
  private func startBattle(run: RunManager) {
    ...
    let freshDeckManager = BattleDeckManager(startingDeck: run.baseDeck)
    battleManager = BattleManager(
      player: run.player, enemies: enemiesToFight, relics: run.relics, deckManager: freshDeckManager
    )
